...
func (h *Handlers) GenerateTransactionsReport(w http.ResponseWriter, r *http.Request) {
	...
	objectPath, total, err := h.repository.GenerateTransactionsReport(r.Context(), startDate, endDate)
	if err != nil {
		h.logsService.Log(r.Context(), config.PERMISSION_FM_TRANSACTIONS, accountID,
			logs.WithStatus(logs.LogStatusError),
			logs.WithUserAgent(r.UserAgent()),
			logs.WithMetadata("error", err.Error()),
			logs.WithMetadata("start_date", startDateStr),
			logs.WithMetadata("end_date", endDateStr),
		)
	...
	presignedURL, err := h.repository.mediaService.GeneratePresignedURL(r.Context(), objectPath, 1*time.Hour)
	if err != nil {
	...
	h.logsService.Log(r.Context(), config.PERMISSION_FM_TRANSACTIONS, accountID,
		logs.WithStatus(logs.LogStatusSuccess),
		logs.WithUserAgent(r.UserAgent()),
		logs.WithMetadata("start_date", startDate.Format("2006-01-02")),
		logs.WithMetadata("end_date", endDate.Format("2006-01-02")),
		logs.WithMetadata("total_transactions", total),
		logs.WithMetadata("object_path", objectPath),
	)
