...
func (s *Service) CreateTransaction(ctx context.Context, companyID, accountID string, amount *int, currency Currency, planCode, nextPlanCode *string, isTrial bool, expiresAt time.Time) (*PricingTransaction, error) {
	...
	query := `
		INSERT INTO pricing_transactions (
			...
			plan_code, is_trial, next_plan_code, expires_at, created_at, updated_at
